...
export function layoutSlots(form, n) {
  ...
  if (form === "横陣") {
    ...
    /* 鶴翼は包み込む陣なので、七つのうちで最も正面が広くなければならない。
       ...
       三.四では横陣と並んでしまい、翼を張った形に見えなかった（実測二.〇対一）。
       六.五で、もとの鶴翼の広さと薄さに戻る（三千六百人で正面三百二m・六段、
       一万人で四百九十七m・九段）。 */
    const per = 横に並べる数(n, 6.5, 4, 26);
    for (let i = 0; i < n; i++) {
...
/* ------------------------------------------- 丘ひとつに一隊（GDD 8.6）
   ...
   丘へは、本隊で登るか、遠隔兵を割いて登らせるか、いずれかである。両方を
   やれば同じ頂へ二重に登ることになる。頂は狭いのだから、翼は裾へはみ出し、
   高みの利は得られぬまま、野に構えるべき戦列だけが薄くなる。

   どちらの形を取るかは、兵の多寡による。

     守勢の隊（受け手であって、しかも兵で優らぬ側）
       … 本隊ごと登って要塞とする。高みは弱者の頼りである。数で押せぬのだから、
         地の利を借りるほかない。この形では遠隔兵を割かない。
     押して出る側（寄せ手、または兵で優る受け手）
       … 本隊は野で敵と当たり、遠隔兵だけを割いて丘へ上げる。数で優るのだから、
         正面を厚く保ったまま高みも取れる。逆に、兵で優らぬ寄せ手は丘に構わない。
         正面で拮抗しているところから兵を割いては、そこから崩れる。

   いずれの形であれ、丘ひとつに一隊である。押さえた丘は b.丘の主 に控えておく
   （丘の番号：押さえた隊の id）。押さえた隊が討たれるか、崩れるか、退くか、
   丘の任を解かれたときにだけ、その丘は空く。 */

// 軍としての優劣。隊ごとではなく、軍全体の兵力（士気ぶんの色を付けたもの）で測る。
export function 兵で優る(b, side) {
  const 量 = (s) => b.corps
    .filter((o) => o.side === s && !o.dead && !o.destroyed && !o.routed)
    .reduce((a, o) => a + corpsMen(o) * (0.7 + o.morale / 330), 0);
  return 量(side) > 量(side === "P" ? "E" : "P") * 1.2;
}

// 守勢の隊か。受け手であって、しかも兵で優らぬ側である。
export const 守勢の隊 = (b, c) => c.side !== b.attacker && !兵で優る(b, c.side);

// その丘が空いているか。押さえ手が退場したなら空く。
export function 丘が空く(b, i, c) {
  const 主 = (b.丘の主 || {})[i];
  if (主 == null || 主 === c.id) return true;
  const o = b.corps.find((x) => x.id === 主);
  if (!o || o.dead || o.destroyed || o.routed || o.withdraw) return true;
  return !!(o.detach && o.task !== "弓鉄砲高地占拠");   // 任を解かれた分遣は手を放す
}

/* いちばん近い空き丘の番号（無ければ −1）。

   いま足を掛けている丘が空いているなら、その丘を選ぶ。これを分けないと、
   麓に立った隊が「自分は丘にいる」と判じて、そこで守りに就いてしまう。
   斜面の裾は高みではない。 */
export function 空き丘を探す(b, c) {
...
/* 分遣を出す頃合い（GDD 8.5）。
   ...
   誰が、いつ、どこで、を見て判ずる。
     ...
     弓鉄砲高地占拠 … 押して出る側だけが割く（上の「丘ひとつに一隊」）。
                      守勢の隊は本隊ごと登るので、そのうえ遠隔兵を割いては
                      同じ丘へ二重に登ることになる。兵で優る受け手は自陣の側の
                      近い高みへ、寄せ手は敵陣の間近まで進んでから。
                      いずれも、まだ誰も押さえていない丘に限る。
...
export function 分遣の頃合い(b, c, key) {
  ...
  if (key === "弓鉄砲高地占拠") {
    ...
    /* 守勢の隊は本隊ごと丘へ登る（ai.js の「高みを取る」）。そのうえ遠隔兵を
       割いては、同じ丘へ二重に登ることになる。分遣で丘を取るのは押して出る
       側であり、しかも兵で優ると見たときだけである。正面で拮抗している
       ところから兵を割いては、そこから崩れる。 */
    if (守勢の隊(b, c)) return false;
    if (!兵で優る(b, c.side)) return false;
    const 番 = 空き丘を探す(b, c);              // すでに誰かが押さえた丘へは出さない
    if (番 < 0) return false;
    const 丘 = HILLS[番];
    const 丘まで = Math.hypot(丘.x - c.x, 丘.y - c.y);
    if (!攻め手) {
      // 兵で優る受け手は、自陣の側の近い高みへ（遠い丘へ兵を割いても戦列が薄くなるだけ）
...
export function detachAI(b, c, alive) {
  ...
  if (c.task === "弓鉄砲高地占拠") {
    /* 押さえた丘を持ち続ける。丘ひとつに一隊であるから、押さえ損ねていたなら
       次に近い空き丘へ移り、空き丘が無ければ本隊へ帰る。 */
    let 番 = c.丘番;
    if (番 == null || 番 < 0 || !丘が空く(b, 番, c)) 番 = 空き丘を探す(b, c);
    if (番 < 0) { c.丘番 = null; c.task = "帰陣"; return true; }   // 空いた高地のない野もある
    c.丘番 = 番; 丘を押さえる(b, 番, c);
    const h = HILLS[番];
