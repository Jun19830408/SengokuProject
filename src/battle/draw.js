...
/* ==========================================================================
   ...
   さも確かなように描くわけにもいかない。
   ========================================================================== */
const 描ける = new Set(["木瓜", "横木瓜", "三つ盛木瓜", "一つ引両", "二つ引両", "三つ引両", "四つ目結", "平四つ目結", "三つ巴", "二つ巴", "一文字三星", "三つ星", "月星", "七曜", "九曜", "四つ割菱", "割り菱", "大内菱", "三階菱", "葵", "酢漿草", "下がり藤", "梅鉢", "撫子", "桐", "沢瀉", "桔梗", "柏", "唐花", "赤鳥", "三つ鱗", "丸に十", "扇", "檜扇", "折敷に三文字", "日足", "杏葉", "二頭立波", "竹に雀", "繋ぎ馬", "州浜", "亀甲", "鷹の羽", "輪宝", "八咫烏"]);

/* 紋の名は修飾を伴う（GDD 13.1）。

   「丸に三つ柏」も「三つ柏」も「柏」も、図柄の核は柏である。ところが名を
   そのまま照らしていたので、核が同じでも名が違えば既定の丸になった。
   増補で立てた家の紋が十五も描けず、みな同じ丸に見えていた。城を攻め取っても
   紋が変わらぬように見えたのは、これである（本山「丸に三つ柏」も、
   奪った長宗我部「酢漿草」も、片方が丸のまま）。

   核だけを取り出して照らす。まず名そのもの、次に「丸に」を外したもの、
   次に数の修飾（一つ・三つ・七つ・五七・違い）を外したもの、最後に
   図柄の近い紋へ寄せる。それでも知らぬものだけが既定の丸になる。 */
const 紋の寄せ = {
  楢の葉: "柏", 梶の葉: "柏", 橘: "唐花", 松皮菱: "三階菱", 立ち葵: "葵",
};
export function 紋の核(kind) {
  if (!kind) return "";
  const 試 = [kind];
...
export function drawMon(ctx, kind, x, y, r, col, sub) {
  ...
  /* 名を核へ解く。描ける紋に当たったところで、その名を使う。
     「丸に」で始まる紋には、外を囲む輪を添える（circle を使うので、
     その定義より後ろに置く）。 */
  const 丸に = /^丸に/.test(kind || "");
  for (const k of 紋の核(kind)) if (描ける.has(k)) { kind = k; break; }
  if (丸に) {
    const 太 = ctx.lineWidth;
    ctx.lineWidth = Math.max(0.7, r * 0.09); circle(0, 0, r * 0.86, false);
    ctx.lineWidth = 太;
  }
