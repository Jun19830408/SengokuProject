...
export function captureChance(gen) {
  ...
  /* 下限は零にしない。どれほどの器量でも、絶対はない。
     ただし四厘では当主の分が下限に食われて効かなくなるので、一厘五毛まで下げる。 */
  return clamp(CAPTURE_BASE * k * 難を逃れる(gen), 0.0015, 0.2);
}

/* 難を逃れる力（GDD 12.3）。小さいほど助かる。

   城が落ちたからといって、そこにいた将がみな討たれ、あるいは縄目を受けるわけではない。

   当主  … 家の重みが違う。配下は身を捨てて殿を務め、当主を落ち延びさせる。
           大名が城とともに討死するのは、まことに稀な事である。
   器量者… 統率に優れれば手勢をまとめて囲みを破り、武勇に優れれば斬り抜け、
           知略に優れれば抜け道を知っている。三つとも高い者はまず捕まらない。

   これまでは、討死の判じに器量がまるで入っていなかった（武勇はむしろ討死を
   増やす向きに働いていた。踏みとどまるからである）。当主の別も無かった。
   一城落ちるたびに一割余りの将が討たれ、名のある者から先に盤を去っていた。 */
export function 難を逃れる(gen) {
  if (!gen) return 1;
  const able = (gen.lead + gen.valor + gen.wit) / 3;
  // 能力40で1.06、70で0.71、95で0.42ほど
  let k = clamp(1.53 - able / 80, 0.30, 1.30);
  if (gen.lord) k *= 0.30;                    // 当主。配下が全力で守って落ち延びさせる
  return k;
}
