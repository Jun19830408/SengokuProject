...
/* 旗頭が攻めたい城を見立てる（GDD 6.4）。
   ...
   他家の城を選ぶ。旗の下（自家・臣従）の城は攻めない。

   大名が攻める家を指していれば、その家の城だけを見る。指していなければ、
...
export function 旗頭の狙い(s, 旗, { 道: 道を引く, 旗の下, 受け持ち, 的家 } = {}) {
  ...
  const 指した家 = 的家 ? 的家(s, 旗) : [];
  /* 狙うのは、受け持ちに隣接する敵から順である（GDD 6.4）。

     もとは「隣り合う城だけ」を見て、そのうち最も守りの薄いものを選んでいた。
     二つの難があった。一つは、隣り合う敵城が一つも無ければ何もしなかったこと。
     受け持ちの内側が固まると、旗頭は永久に動かない。もう一つは、遠近を問わず
     守りの薄さだけで選んでいたので、受け持ちの反対側の城へ向かうことがあったこと。

     領土に近い敵から順に当たる。同じ近さなら、守りの薄いほうから。
     境を接する敵を片づけてから次へ進むのが、方面軍を預かる者の務めである。 */
  const 見 = [];
  for (const 的 of s.castles) {
    ...
    /* 受け持ちから遠すぎる城は預かりの外。ただし大名がその家を指しているなら、
       多少遠くとも向かう。下知があるのに「遠いので動きません」では方面軍ではない。
       実測では、受け持ちに国境を接していても道で五歩かかる家があり、指しても
       旗頭が何もしなかった。 */
    if (近 == null || 近 > (指した家.length ? 8 : 4)) continue;
...
/* 旗頭に預ける高。受け持ちの国々にある自領の城の実入りに、目盛りを掛ける。

   受け持ちとは、己の国と寄騎たちの国である。方面軍を預けるとは、その国々の
   差配を預けることだからである（寄騎はそのうち、政務まで委ねた城のことである）。 */
export function 旗頭の預け高(s, 旗, { 受け持ち } = {}) {
  const f = (s.factions || {})[旗.faction] || {};
  const 受 = 受け持ち ? 受け持ち(s, 旗) : [];
  const 城ら = (s.castles || []).filter((c) => c.faction === 旗.faction && 受.includes(c.kuni));
