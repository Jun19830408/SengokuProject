...
/* 従える側に要る石高の比（GDD 12.1 / 12.5）。返すのは「相手が自家の何割まで」。

   位なし　　　　　… 四割（＝自家が二.五倍）
   右大臣・内大臣　… 七割五分（＝一.三三倍）。五畿を制した者の格
   征夷大将軍　　　… 一.三五（＝自家より大きい家も従えられる）
                     ただし直轄が全国の一割を要る。幕府の権威は、それだけの
                     身代を背負ってはじめて意味を持つ。 */
export const 天下人の目安 = 0.10;                   // 直轄が全国に占める割合
export const 従える比 = (me) => {
  if (me.官位 === "征夷大将軍" && me.koku >= (me.全国 || 0) * 天下人の目安) return 1.35;
  if (me.官位) return 0.75;
  return 0.40;
};

/* 臣従させる側に要る比。官位で緩むが、一倍を越えることはない。
   自分より大きい家を旗の下へ完全に入れることは、天下人にもできない。 */
export const 臣従させる比 = (me) => {
  if (me.官位 === "征夷大将軍" && me.koku >= (me.全国 || 0) * 天下人の目安) return 0.80;
...
export const DIPLO = [
  ...
  { key: "従属させる", cost: 400, state: "従属", dir: "上",
    ...
    need: (r, me, you) => !SUBJECT.includes(r.state) && !me.旗の下か
      && you.koku < me.koku * (従える比(me) + (me.diplo || 0) * 0.012)
      && r.trust >= 80 - (me.diplo || 0) - 威信の効き(me) },
  ...
  { key: "臣従させる", cost: 900, state: "臣従", dir: "上",
    ...
    need: (r, me, you) => !SUBJECT.includes(r.state) && !me.旗の下か
      && you.koku < me.koku * Math.min(0.80, 臣従させる比(me) + (me.diplo || 0) * 0.010)
      && r.trust >= 100 },
  ...
  { key: "従属する", cost: 0, state: "従属", dir: "下",
    ...
    need: (r, me, you) => !SUBJECT.includes(r.state) && !me.旗の下か
      && you.koku > me.koku * 2.5 && r.trust >= 80 },
...
export const PLOTS = [
  ...
  { key: "離間", cost: 360, months: 3, need: 86, cap: 0.80, hard: 0.68,
    mato: "無", 家を指す: true,
