...
export function 外交の采配(s, fid, { 告げる, 申し入れる } = {}) {
  ...
  const 打つ = (相, key) => {
    ...
    /* 使者には、政務と知略の勝る者を立てる。親善の効き目はその器量で決まる。
       遊ぶ側と同じ決まりで動かねば、誼の篤くなる早さが釣り合わない。 */
    const 使 = s.generals.filter((x) => x.faction === fid && !x.captive)
      .sort((x, y) => ((y.gov || 0) + (y.wit || 0)) - ((x.gov || 0) + (x.wit || 0)))[0];
    const r = 外交を結ぶ(s, fid, 相, key, 使 ? 使.id : null);
    if (r.ok && 告げる) 告げる(r.文);
  ...
  if (大物) {
    ...
    /* 早めに膝を屈する（GDD 12.1 / 13.2）。

       もとは「城一つになるまで粘り、十度に一度しか踏み切らない」であった。
       それでは滅ぶまで独りで戦い続けることになり、地方はいつまでも小家で
       埋まったままになる。戦国の国人は、もっと早く旗の下に入った。
       浅井が六角に、真田が武田に、松浦が大友に付いたのはそういう形である。

       関門（DIPLO の need）は動かさない。従属には信用八十が要り、臣従には
       石高二.六倍の差が要る。ここで緩めるのは「踏み切る気になるか」だけである。 */
    if (差 >= 3.0 && (自城.length <= 3 || 狙われ) && 引く() < 0.22) {
  ...
  // 要る信用は 従属させる八十・臣従させる百（GDD 12.1）。采配の目安もそこへ合わせる。
  if (小物 && 小物.r.trust >= 80 && 引く() < 0.30) {
  ...
  /* 四、背後を固める。狙う相手がいるなら、その相手でない隣家と誼を通じ、
     ...
     誼には落ち着き所（四十五）があり、放っておけば薄れていく。方々へ均等に
     使者を出しても、どこも落ち着き所の周りで足踏みするだけで、同盟にも婚儀にも
     届かない。積むなら一家に絞る。

     しかも、通う先と結ぶ先は同じ家でなければならない。もとは石高のいちばん
     大きい隣家（背後）へ同盟を持ちかけ、誼はいちばん篤い家へ通わせていた。
     二十五年走らせて測ると、新たに結ばれた同盟は零であった――通う先で信用が
     積まれても、結ぶ先はいつまでも薄いままだからである。 */
  const 通う先 = (() => {
    /* 一　己より遙かに大きい隣家に脅かされているなら、そこへ通う。
           生き残りが先で、同盟づくりは後まわしになる。実際そういうものである。 */
    if (大物 && !["同盟", "臣従", "従属"].includes(大物.r.state)) {
      const 差 = 大物.koku / Math.max(1, 我石);
      if (差 >= 1.9) return 大物;
    }
    /* 二　呑めそうな小家があれば、そこへ通う。ただし外交の手は限りがあるので、
           半ばは背後を固めるほうへ回す。ここを全部小家へ振ると、二十五年で
           新たに結ばれる同盟が零になった（tests/gaiko.cjs が捕まえた）。 */
  ...
  /* 親善は意図に向ける（GDD 11.1 / 12.1）。

     従属も臣従も、要るのは信用である（従属八十、従える側は八十と百）。
     ところが親善は「いちばん誼の篤い隣家」へ均等に出していた。誼を積む先と、
     旗の下に入りたい先が食い違っていては、いつまでも関門を越えない。

     ゆえに向け先を意図で決める。
       一　己より遙かに大きい隣家があれば、そこへ通う（膝を屈するために）
       二　呑めそうな小さい隣家があれば、そこへ通う（従えるために）
       三　どちらも無ければ、これまで通り背後を固める

     一と二はどちらも「上下の間柄を結ぶための親善」である。攻めずに勢力圏を
     広げる道であり、戦だけで地方が絞られるより、よほど戦国らしい。 */
  // いちばん誼の篤い隣家。同じ相手へ通い、同じ相手と結ぶ
  const 相 = [...相手ら].sort((a, b) => b.r.trust - a.r.trust)[0]
    || 隣.filter((x) => x.先 !== 狙)[0];
  if (相) {
    if (相.r.trust >= 65 && 引く() < 0.5) {
      const r = 打つ(相.先, "同盟") || 打つ(相.先, "不可侵");
      if (r) return r;
    }
    ...
    /* 親善の向け先。意図があるならそちらへ、無ければ背後の家へ。
       誼を積む先と、結びたい先が食い違っていては、いつまでも関門を越えない。 */
    const 先 = 通う先 || 相;
    const r = 打つ(先.先, "親善");                 // 信用を積む。いちばん多い手である。
    if (r) return r;
  }
  return null;
}
