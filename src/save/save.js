...
/* 棚が一杯になったときの繕い（GDD 15.3）。
   ...
   置き直す。中身は一字も変えない。詰め直せば五分の一以下になるので、たいていは
   これで空きができる。そのうえで、もう一度だけ書いてみる。

   ただし「圧して上書きする」だけでは足りない。棚が大きく溢れていると、
   置き換えたあとの総量もなお溢れているので、その一枠さえ撥ねられる。
   一枠も詰め直せず、以後どの枠へも書けない。盤が育って一枠が一.二六MBから
   一.五六MBになった途端、これが起きた（六.三MBの棚で、詰め直しは六度
   撥ねられ、一バイトも減らなかった）。

   ゆえに二段構えとする。
     一の手 … これまでどおり、圧したもので上書きする。棚から下ろさないので、
              撥ねられても何も失わない。たいていはこれで空きができる。
     二の手 … 一の手で一枠も詰められなかったときだけ。生のまま入っている枠を
              いったんすべて手元へ取り出し、まとめて下ろしてから置き直す。
              まとめて下ろせば棚はほぼ空になるので、置き直しは必ず通る。

   はじめは一の手を「下ろしてから置き直す」形にしていた。ところが撥ねられて
   元の記録を書き戻そうとすると、その書き戻しもまた撥ねられる（棚はなお
   溢れたままだからである）。下ろした枠が、そのまま失われた。
   下ろしてよいのは、戻せる見込みが立つときだけである。

   いずれの手でも、下ろした中身は手元に持っている。置き直せなかった枠には
   元の記録を書き戻す。記録を失わせないことが、何よりも先である。 */
async function 圧して詰める(key) {
  let 文 = null;
  try { 文 = await 置き場().読む(key); } catch (e) { return null; }
  if (!文 || 文.startsWith("z1:")) return null;        // すでに圧してある
  const d = 解く(文);
  if (!d || !d.state) return null;                     // 読めぬものは触らない
  const 圧 = 圧す(JSON.stringify({ v: d.v || 版, at: d.at || Date.now(), state: d.state }));
  /* 下ろす前に、圧したものが読み戻せることを確かめる。
     読み戻せぬものと引き換えに元の記録を下ろしては、本末転倒である。 */
  let 検 = null;
  try { 検 = JSON.parse(解す(圧)); } catch (e) { return null; }
  if (!検 || !検.state || !Array.isArray(検.state.castles)) return null;
  if (検.state.castles.length !== (d.state.castles || []).length) return null;
  return { key, 生: 文, 圧 };
}
...
async function 棚を詰め直す(除く) {
  ...
  for (const w of 枠一覧()) {
    ...
    const h = await 圧して詰める(w.key);
    if (h) 手元.push(h);
  }
  if (!手元.length) return 0;

  // 一の手。棚から下ろさずに上書きする。撥ねられても何も失わない。
  let 詰めた = 0;
  const 残り = [];
  for (const h of 手元) {
    try { await 置き場().書く(h.key, h.圧); 詰めた++; }
    catch (e) { 残り.push(h); }
  }
  if (詰めた) return 詰めた;

  /* 二の手。一枠も通らなかった。まとめて下ろしてから置き直す。
     手元にはすべての中身があるので、置き直せなかったぶんは書き戻す。 */
  for (const h of 残り) { try { await 置き場().消す(h.key); } catch (e) { /* 次へ */ } }
  for (const h of 残り) {
    try { await 置き場().書く(h.key, h.圧); 詰めた++; }
    catch (e) { try { await 置き場().書く(h.key, h.生); } catch (e2) { /* 失うほかない */ } }
  }
  return 詰めた;
}
