...
}
{
  ...
  確('高地占拠は受け手が先に出す', 丘.守 > 丘.攻,
    `受け手${丘.守}・寄せ手${丘.攻}`);
  /* 丘ひとつに一隊と定めてから（ai.js の「高みを取る」）、この割は少し下がった。
     本隊が丘を取れなかったとき、その隊は野に構えたまま、あとになって遠隔兵を
     空き丘へ割く。丘を取り合う隊が減ったぶん空き丘が残るので、高地占拠の分遣
     そのものは増えている（十六戦で二十三→二十八回。うち受け手二十→二十三）。
     つまり丘の取り合いは分遣の判じを鈍らせていない。遅れて出るものが増えて、
     初めに出る割が下がっただけである。

       賽を四通り振って、噛み合う前に出た割
         一丘一隊の前   52%・60%・71%・52%
         一丘一隊のあと 50%・62%・50%・54% */
  確('受け手の高地占拠は、半ばは戦の初めに出る', 丘.触前 >= 丘.計 * 0.45,
    `${丘.触前}／${丘.計}回が噛み合う前（一丘一隊の前は12／23回）`);
