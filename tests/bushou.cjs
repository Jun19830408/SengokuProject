...
/* ------------------------------------------------ 一、名と id の重なり */
{
  ...
  /* 家を跨いだ同名は、咎めずに並べるだけにする。

     若狭武田の信豊と武田信繁の子の信豊のように、家が違えば別人でありうる。
     いっぽうで七里頼周のように、本願寺の坊官が加賀へ下って一揆を率いた者を、
     二つの家に別々に置いてしまうこともある（実際そうなっていて、第八回で
     気づいた）。機械には見分けられないので、増補のたびに目で確かめる。 */
  const 跨名 = {};
  for (const g of 全) (跨名[素の名(g.name)] = 跨名[素の名(g.name)] || []).push(`${g.id}/${g.faction}`);
  const 跨り = Object.keys(跨名).filter((k) => 跨名[k].length > 1);
  console.log(`  （家を跨いだ同名 ${跨り.length}組：${跨り.slice(0, 8)
    .map((k) => `${k}(${跨名[k].join('・')})`).join(' ')}${跨り.length > 8 ? ' …' : ''}）`);
}
