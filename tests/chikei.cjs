/* 地物を避けて寄せる（GDD 8.1 / 8.6）。
   ...
   のち、丘と川の扱いを二度にわたって改めた。

   一、丘ひとつに一隊（ai.js の「高みを取る」／corps.js の「丘ひとつに一隊」）。
       受け手の全隊が同じ頂へ折り重なっていた。丘へは本隊で登るか、遠隔兵を割いて
       登らせるか、いずれか一つである。守勢の隊は本隊ごと登って要塞とし、押して
       出る側は本隊を野に置いて分遣で取る。寄せ手の本隊はもう丘に登らない。

   二、川の掟（ai.js）。丘に籠る隊が減って野で当たるようになったぶん、決戦の場が
       川際へ移り、水の上で槍を合わせる戦がかえって増えた。行き先を決めたら
       最後に一度、川の掟を通すことにした（下の「六」）。

   同じ二十四戦（同じ賽・同じ野）で、三つの時点を測った。

                          作業前    一丘一隊    川の掟
     受け手が丘に就く     62／72隊   41／72隊   41／72隊
     同じ丘への折り重なり    24件       2件        3件
     淵で過ごす割           3.8%      5.2%       3.0%
     水で過ごす割           4.5%      7.1%       3.7%
     地物で過ごす割        12.1%     13.8%      11.1%
     水の中で槍を合わす      5.4%      9.6%       0.4%
     渡り場を通った渡河   43／48回   37／38回   42／42回

   丘の折り重なりは、分遣まで数に入れると 480件 → 20件 になった。もとは本隊と、
   その本隊が割いた分遣とが同じ頂に登っているものが大半であった。

   勝敗も測った。同じ二十四戦で寄せ手の勝ちが 5 → 9 → 12、受け手の勝ちが
   18 → 13 → 12 である。受け手が丘に籠り、寄せ手が水の中で叩かれる、という
   一方に傾いた戦が減って五分に近づいた。損害も 寄せ手89,500／受け手74,740 から
   寄せ手83,232／受け手83,170 になった。 */
...

{
  ...
  確('淵の中で過ごさない', 割((r) => r.淵刻) < 5,
    `${割((r) => r.淵刻).toFixed(1)}%（道さがしの前は8.0%、川の掟の前は5.7%）`);
  確('水の中で過ごす刻が減った', 割((r) => r.水刻) < 6,
    `${割((r) => r.水刻).toFixed(1)}%（道さがしの前は11.8%、川の掟の前は8.8%）`);
  確('森・林・湿地・集落を避けて通る', 割((r) => r.障刻) < 13,
    `${割((r) => r.障刻).toFixed(1)}%（道さがしの前は16.9%）`);
  確('渡河のほとんどは橋か浅瀬を通る', 和((r) => r.渡り場) > 和((r) => r.渡り) * 0.9,
    `${和((r) => r.渡り場)}／${和((r) => r.渡り)}回（道さがしの前は67%、川の掟の前は90%）`);
  /* 川の掟（ai.js）を入れてから、水の上で槍を合わせることはほとんど無くなった。
     受け手は渡り場のこちら岸で待ち、寄せ手は渡り切ってから当たる。掟の前は
     一割以上あった。閾も、得られた保証に合わせて締め直す。 */
  確('水の中で槍を合わせない',
    和((r) => r.戦) === 0 || 和((r) => r.水戦) / 和((r) => r.戦) < 0.02,
    `${(和((r) => r.水戦) / Math.max(1, 和((r) => r.戦)) * 100).toFixed(1)}%（道さがしの前は12.1%、川の掟の前は12.7%）`);
...
function 混ませる(知) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    /* 数えるのは「橋の混みを嫌って瀬へ乗り入れた」ぶんだけである。

       もう一つ、睨み合いのまま日が暮れるのを避ける掟がある（ai.js の
       「渡り場を探しあぐね、淵を押し渡る」）。これは寄せ手の権であって、
       知略に依らない――待つのは受け手の権であり、寄せ手は渡らねば何も
       始まらないからである。両方を一緒に数えると、この節が測りたい
       「知略の将だけが橋待ちを嫌って瀬を渡る」が見えなくなる。 */
    if (b.log.some((x) => /橋の混みを嫌い/.test(x.text))) 押し++;
    if (b.log.some((x) => /探しあぐね/.test(x.text))) 睨み++;
  }
  return { 押し, 戦, 睨み };
}
{
  ...
  確('知略に富む将は、橋が混めば瀬を押し渡る', 知.押し > 0,
    `${知.押し}／${知.戦}戦`);
  確('知略の足りぬ将は、橋待ちを嫌って瀬へ乗り入れたりしない', 凡.押し === 0,
    `${凡.押し}／${凡.戦}戦`);
  /* もう一つ、睨み合いのまま日が暮れるのを避ける掟がある（寄せ手の権であって
     知略に依らない）。これは渡り場へ寄る道がどうしても引けぬときの最後の手段
     なので、道がちゃんと引けている限り出ないほうがよい。数だけ控えておく。 */
  console.log(`    （睨み合いの果ての渡河：知略の将 ${知.睨み}戦／凡将 ${凡.睨み}戦）`);
}
...
/* ------------------------------------------------- 六、川の掟（GDD 8.1）
   ...
   川で槍を合わせるのは双方にとって愚である。足は0.1、戦う力は半分、隊列は
   二十四も削れる。それでも盤の上では、交戦の一割以上が水の上で起きていた。
   川を避ける道理を采配のあちこちに置いていたので、必ず漏れが出た（追い討ちの
   段だけが道理を素通りしていて、崩れた敵を追って川へ入っていた）。

   いまは行き先を決めたら最後に一度、川の掟を通す。掟は四つ。
     一、受け手は川を渡らない。渡り場のこちら岸で待つ。
     二、寄せ手は渡り場を通って渡り切る。瀬の上では止まらない。
     三、どちらであれ、行き先が水にかかるなら岸へ置き直す。噛み合った隊にも
         及ぼす（噛み合っていても隊は行き先へ歩くからである）。
     四、狙う敵が水の中にいるなら、追って入らない。岸に構えて射る。

   縛るのは采配だけである。手ずから「川を渡れ」と命じたなら渡らせる。 */
{
  種 = 0x5150;
  A.setBattleMap(null);
  let 見 = null;
  for (let i = 1; i < 60 && !見; i++) {
    A.setFieldSeed('kw' + i, 'x'); A.layoutField(9000, 6);
    if (A.hasRiver()) 見 = i;
  }
  const x0 = A.FIELD.w * 0.5;
  const 中 = (A.RIVER.top + A.RIVER.bot) / 2 + A.riverShift(x0);
  const 半 = (A.RIVER.bot - A.RIVER.top) / 2;

  // 一と二。川を挟んで向かい合わせ、受け手が渡らず、寄せ手が渡ることを見る。
  {
    const E = [0, 1, 2].map((k) => A.makeCorps('E', 将(10 + k), 400, 1100, 75, 75,
      x0 + (k - 1) * 260, 中 - 半 - 420, Math.PI / 2, '#B0483C'));
    const P = [0, 1, 2].map((k) => A.makeCorps('P', 将(k), 400, 1100, 75, 75,
      x0 + (k - 1) * 260, 中 + 半 + 700, -Math.PI / 2, '#2F5D8C'));
    for (const c of [...P, ...E]) { c.formation = '横陣'; A.placeSquads(c, true); c.auto = true; }
    const b = A.createBattle(P, E, 'P');
    b.mode = 'field'; b.phase = 'fight'; b.dusk = 4000; b.face = 'S'; b.myFar = false;
    let 受けが水に = 0, 刻 = 0;
    ...
    for (let k = 0; k < 1600; k++) {
      ...
      if (k % 3 === 0) A.battleAI(b);
      for (const c of E) {
        // 崩れて逃げる隊が水へ飛び込むのは采配の働きではないので数えない
        if (c.dead || c.destroyed || c.routed) continue;
        刻++;
        if (水.has(c.地 || A.terrainAt(c.x, c.y))) 受けが水に++;
      }
      for (const c of P) if (!c.dead && !c.destroyed && A.岸(c.x, c.y) === -1) 渡った.add(c.gen.id);
      if (b.result) break;
    }
    確('受け手は、敵が向こう岸にいても川を渡らない', 受けが水に < 刻 * 0.02,
      `水にかかった刻 ${受けが水に}／${刻}`);
    確('寄せ手は渡り場を通って向こう岸へ渡る', 渡った.size > 0,
      `${渡った.size}／3隊が渡り切った`);
  }

  // 四。水の中の敵を追わず、岸から射ること。
  {
    const 寄 = A.makeCorps('P', 将(1), 400, 1100, 75, 75, x0, 中, -Math.PI / 2, '#2F5D8C');
    const 受 = A.makeCorps('E', 将(11), 400, 1100, 75, 75, x0, 中 - 半 - 130, Math.PI / 2, '#B0483C');
    for (const c of [寄, 受]) { c.formation = '横陣'; A.placeSquads(c, true); c.auto = true; }
    const b = A.createBattle([寄], [受], 'P');
    b.mode = 'field'; b.phase = 'fight'; b.dusk = 4000; b.face = 'S'; b.myFar = false;
    let 射った = false, 水に = 0, 刻 = 0;
    for (let k = 0; k < 200; k++) {
      A.stepBattle(b, 0.25);
      if (k % 3 === 0) A.battleAI(b);
      if (受.order === '射撃') 射った = true;
      if (!受.routed) { 刻++; if (水.has(受.地 || A.terrainAt(受.x, 受.y))) 水に++; }
      if (b.result) break;
    }
    確('水の中の敵は追わず、岸から射る', 射った, `受け手の命は「${受.order}」`);
    確('射るあいだも受け手は水に入らない', 水に === 0, `水にかかった刻 ${水に}／${刻}`);
  }

  // 五。手ずから命じたなら、受け手でも川へ入る。
  {
    const 手 = A.makeCorps('P', 将(2), 400, 1100, 75, 75, x0, 中 - 半 - 200, Math.PI / 2, '#2F5D8C');
    const 敵 = A.makeCorps('E', 将(12), 400, 1100, 75, 75, x0, 中 + 半 + 600, -Math.PI / 2, '#B0483C');
    for (const c of [手, 敵]) { c.formation = '横陣'; A.placeSquads(c, true); c.auto = true; }
    const b = A.createBattle([手], [敵], 'E');        // 寄せ手は E。P が受け手である
    b.mode = 'field'; b.phase = 'fight'; b.dusk = 4000; b.face = 'S'; b.myFar = false;
    A.issueOrder(b, 手, { order: '移動', tx: x0, ty: 中 });   // 川の真ん中へ、という指図
    let 入った = false;
    for (let k = 0; k < 200; k++) {
      A.stepBattle(b, 0.25);
      if (k % 3 === 0) A.battleAI(b);
      if (A.岸(手.x, 手.y) === 0) 入った = true;
      if (b.result) break;
    }
    確('手ずから命じれば、受け手でも川へ入る（掟は采配だけを縛る）', 入った,
      入った ? '指図どおり水へ入った' : `岸${A.岸(手.x, 手.y)}に留まった`);
  }
  A.setBattleMap(null);
}
...
console.log('\n── 淵は、決めてからでなければ踏み込まない');
{
  let 淵人 = 0, 総人 = 0, 川の野 = 0, 押し渡り = 0, 渡った = 0, 隊数 = 0;
  for (const 鍵 of [11, 22, 33, 44, 55, 66]) {
    ...
    const 中川 = (A.RIVER.top + A.RIVER.bot) / 2;
    const 岸0 = new Map(b.corps.map((c) => [c, c.y < 中川 ? -1 : 1]));
    ...
    for (const c of b.corps) { 隊数++; if ((c.y < 中川 ? -1 : 1) !== 岸0.get(c)) 渡った++; }
  }
  const 割 = 淵人 / Math.max(1, 総人) * 100;
  確('川のある野で試せている', 川の野 >= 2, `${川の野}／6`);
  確('決めもせず淵に立つ兵は、ごく僅か', 割 < 1.2,
    `${割.toFixed(2)}％（掟を入れる前は 3.62％）`);
  /* この節が測りたいのは「決めもせず淵に立つ兵はいない」ことである。
     そもそも誰も川へ近寄らなければ、その数字は意味を持たない。
     隊がちゃんと向こう岸へ渡っていることを、裏付けとして見る。
     渡り方は問わない――橋でも浅瀬でも、腹を決めての押し渡りでもよい。 */
  確('隊はちゃんと向こう岸へ渡る（測りが空でない）', 渡った > 隊数 * 0.3,
    `${渡った}／${隊数}隊が渡った（うち押し渡ると決めた標本 ${押し渡り}）`);
}
