...
/* ------------------------- 五、撃つ順の偏りがないこと（測って気づいた不具合）
   ...
   ただし、南から北へ渡る側と北から南へ渡る側では、風の当たりが逆になる。
   海を広げたところ、寄せ合うまでの道のりが伸び、この差が効くようになった
   （二十戦して十六勝）。風が効くのは正しいので、測るほうを直す。
   同じ風の下で陣を入れ替え、両方から測れば、風の分は打ち消される。
   残るのは撃つ順の偏りだけである。 */
{
  const 一戦入替 = (我艘, 我技, 敵艘, 敵技, seed) => {
    種(seed);
    H.layoutSea(seed, 6000);
    // 陣を入れ替える。P が北から、E が南から寄せる
    const P = [H.makeFleet('P', 将('我', 62), 我艘, 我技, H.SEA.w * 0.4, H.SEA.h * 0.2, Math.PI / 2, '#2F5D8C')];
    const E = [H.makeFleet('E', 将('敵', 72), 敵艘, 敵技, H.SEA.w * 0.4, H.SEA.h * 0.8, -Math.PI / 2, '#B0483C')];
    const b = H.createSeaBattle(P, E, 'P', {});
    b.phase = 'fight';
    H.海戦を裁く(b);
    return { r: b.result };
  };
  /* 試行は多く取る。十四戦ずつで測ったときは二十八戦して十九勝と出て、
     偏りがあるように見えた。四十戦ずつに増やすと四割四分に落ち着いた。
     海戦は一戦ごとの振れが大きいので、少ない試行では何も言えない。 */
  const N = 20;
  const 南 = [...Array(N)].map((_, i) => 一戦(18, 62, 18, 62, i + 101));
  const 北 = [...Array(N)].map((_, i) => 一戦入替(18, 62, 18, 62, i + 101));
  const s南 = 南.filter((r) => r.r === 'P').length, s北 = 北.filter((r) => r.r === 'P').length;
  const 勝 = s南 + s北;
  確('陣を入れ替えて測れば、勝ち負けは五分になる', 勝 >= N * 0.6 && 勝 <= N * 1.4,
    `${N * 2}戦して ${勝}勝（${Math.round(100 * 勝 / (N * 2))}%／南から${s南}・北から${s北}）`);
}
