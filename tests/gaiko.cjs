...
/* ---------------------------------------------- 一、遊ぶ側の下知は今まで通り */
{
  ...
  /* 間柄に要る信用（GDD 12.1）。
     ...
     石高の比は、旗の下の要件を上げたので改めた（tests/juzoku.cjs を見よ）。
     従属は二.五倍、臣従は四倍を要る。ここで渡す比は、その関門を通る値にする
     ――信用の関門を測る節であって、石高の関門を測る節ではない。 */
  {
    ...
    確('従属させるは信用八十から', !要る('従属させる', 79, 0.3) && 要る('従属させる', 80, 0.3));
    確('臣従させるは信用満のみ', !要る('臣従させる', 99, 0.2) && 要る('臣従させる', 100, 0.2));
    確('従属するは信用八十から', !要る('従属する', 79, 2.6) && 要る('従属する', 80, 2.6));
    確('臣従するに信用は問わない（降る道を塞がない）', 要る('臣従する', 0, 4.1));
  }
}
...
const s = (() => { let t = A.initState('oda'); t.autoPlay = true; return t; })();
{
  ...
  /* 間柄の一つ一つを控えておく。
     ...
     もとは「同盟の数が増えること」で判じていた。これは仕様ではない。
     二十五年のうちには家も滅びるので、結ばれた同盟より絶えた同盟のほうが
     多い年もある。初めの間柄を書き足して同盟が十一から十七になった途端、
     十七→十六となって倒れた。減ってはいるが、盤は少しも凍っていない。

     見たいのは「間柄が動いたか」であるから、動いた組の数をそのまま数える。
     併せて、絶えるばかりでなく新たに結ばれてもいることを確かめる。 */
  const 初表 = {};
  for (const k of Object.keys(t.relations)) 初表[k] = t.relations[k].state;
  ...
  for (const k of Object.keys(t.relations)) {
    const 前 = 初表[k] || '中立', 今 = t.relations[k].state;
    if (前 !== 今) 動++;
    if (今 === '同盟' && 前 !== '同盟') 新同盟++;
  }
  確('二十五年のうちに間柄が動く', 動 > 30 && 新同盟 > 0 && (後.従 + 後.臣) > (初.従 + 初.臣),
    `動いた組 ${動}／新たに結ばれた同盟 ${新同盟}／`
