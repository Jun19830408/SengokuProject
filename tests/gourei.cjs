...
const 具 = {
  ...
  素の道: (a, b) => H.findPath(a, b),         // 惣無事令を経ていれば道の掟を措く
  兵糧: (人, 月) => 遠征の兵糧(人, 月),
};
...
console.log('');
console.log('── 十　惣無事令を経ていれば、号令の軍は道の掟を措いて進める');
{
  /* 常の出陣では他家の領を素通りできない。ところが号令は違う――惣無事令を発して
     天下の諸大名を旗の下に置いたうえでの下知なのだから、道々の家も同じ旗の下に
     ある。秀吉の小田原に、東海道の大名が道を貸さぬはずがない。

     惣無事令を経ずに道の掟のままだと、参陣できる手の多くが道を引けずに消える。 */
  const 的を選ぶ = (t) => t.castles.find((c) => c.faction !== 'oda'
    && !旗の下の家ら(t, 'oda').includes(c.faction));

  const a = 天下人にする(); 国主を並べる(a);
  const 的a = 的を選ぶ(a);
  const 可a = 号令を発せるか(a, 'oda', 的a.id, { 号令できるか });
  確('惣無事令を経ていないと控える', 可a.触れ済み === false);
  const 号a = 号令を発する(a, 'oda', 的a.id, 可a.手ら, 具);
  const 立a = 号a ? 号a.手.length : 0;

  const b = 天下人にする(); 国主を並べる(b);
  b.惣無事令の控え = { oda: { y: b.year, m: b.month } };      // 惣無事令を発してある
  const 的b = 的を選ぶ(b);
  const 可b = 号令を発せるか(b, 'oda', 的b.id, { 号令できるか });
  確('惣無事令を経ていると控える', 可b.触れ済み === true);
  確('参陣できる手の数は同じ', 可b.手ら.length === 可a.手ら.length,
    `${可a.手ら.length}手`);
  const 号b = 号令を発する(b, 'oda', 的b.id, 可b.手ら, 具);
  const 立b = 号b ? 号b.手.length : 0;
  確('惣無事令を経ていれば、多くの手が立つ', 立b > 立a,
    `道の掟のまま ${立a}手 → 掟を措いて ${立b}手（参陣できる手 ${可a.手ら.length}）`);
  確('経ていれば、ほとんどの手が立つ', 立b >= 可b.手ら.length * 0.8,
    `${立b}／${可b.手ら.length}手`);
  確('経ていなければ、道の掟がそのまま効く', 立a < 可a.手ら.length,
    `${立a}／${可a.手ら.length}手`);
  /* 出られなかった手は控える。黙って消えては、遊ぶ側に理不尽に映る。 */
  確('出られなかった手が控えられる',
    (号a.出られず || []).length === 可a.手ら.length - 立a,
    `${(号a.出られず || []).length}手`);
  確('その訳が読める', (号a.出られず || []).every((x) => !!x.訳),
    ((号a.出られず || [])[0] || {}).訳 || '');
}
