...
}
{
  ...
  /* 立ち直りは、二十戦のうち一度あるかないかの稀な出来事である。
     「一度でも起きたか」で測ると、戦の運びを少し変えるだけで一と零を行き来する
     ――事実、采配に手を入れたら 一／七十四崩れ が 〇／八十崩れ になった。
     当たり外れを見ていては、決まりが働いているのかどうか分からない。

     決まりそのものを直に測る。崩れた隊を敵から遠ざけて置き、士気と兵が
     戻る条件（士気四十以上・兵が四分の一以上・崩れて三十秒）を満たしたとき、
     戦列に返るかを見る。二十戦の数は、下に控えとして出しておく。 */
  console.log(`  （二十戦での立ち直りは ${和((r) => r.立ち直り)}隊。稀なので、下で決まりそのものを測る）`);
  確('二十戦を通しても、崩れた隊は数えられるだけ出る', 和((r) => r.崩) > 20,
    `崩れた ${和((r) => r.崩)}隊`);
  /* ---- 立ち直りの決まりそのものを測る（当たり外れに頼らない）
       崩れた隊を敵から遠ざけて置き、息をつかせる。
       士気四十・兵が四分の一・崩れて三十秒――これを満たせば戦列に返る。 */
  {
    const 仕 = (o) => {
      種 = 0x7700;
      A.setBattleMap(null); A.setFieldSeed('r', 'r'); A.layoutField(9000, 4);
      const W = A.FIELD.w, H = A.FIELD.h;
      const P = [A.makeCorps('P', 将(0), 400, 1100, 75, 75, W * 0.5, H * 0.9, -Math.PI / 2, '#2F5D8C')];
      const E = [A.makeCorps('E', 将(10), 400, 1100, 75, 75, W * 0.5, H * 0.1, Math.PI / 2, '#B0483C')];
      for (const c of [...P, ...E]) { c.formation = '横陣'; A.placeSquads(c, true); c.auto = false; }
      const b = A.createBattle(P, E, 'P');
      b.mode = 'field'; b.phase = 'fight'; b.dusk = 1100;
      const c = P[0];
      // 崩れた形にして、敵から遠ざけたまま置く
      c.routed = true; c.崩れた刻 = b.t; c.morale = o.士気;
      c.order = '待機'; c.tx = c.x; c.ty = c.y;
      if (o.兵を減らす) {
        // 兵を四分の一より下まで減らす（立ち直れぬ形）
        for (const q of c.squads) q.men = Math.max(0, Math.round(q.men * 0.12));
      }
      for (let i = 0; i < 30 * 70; i++) A.stepBattle(b, 1 / 30);      // 七十秒
      return { c, 割: A.corpsMen(c) / Math.max(1, A.corpsMax(c)) };
    };
    const 甲 = 仕({ 士気: 46 });
    確('息をつけば、崩れた隊は戦列に返る',
      甲.c.routed === false && 甲.c.立ち直り数 === 1,
      `士気 ${Math.round(甲.c.morale)}・兵 ${Math.round(甲.割 * 100)}%　→ ${甲.c.routed ? '崩れたまま' : '立ち直った'}`);
    const 乙 = 仕({ 士気: 46, 兵を減らす: true });
    確('兵が戻らぬ隊は返さない（返してもすぐまた崩れる）',
      乙.c.routed === true || 乙.c.潰 === true,
      `兵 ${Math.round(乙.割 * 100)}%　→ ${乙.c.潰 ? '盤を落ちた' : 乙.c.routed ? '崩れたまま' : '★立ち直った'}`);
    // 一度きりであること
    const 丙 = 仕({ 士気: 46 });
    if (丙.c.routed === false) {
      丙.c.routed = true; 丙.c.崩れた刻 = 0; 丙.c.morale = 60;
      const b2 = { corps: [丙.c] };
      確('立ち直れるのは一度きり（控えの数が二にならない）', 丙.c.立ち直り数 === 1,
        `立ち直り数 ${丙.c.立ち直り数}`);
    }
  }
