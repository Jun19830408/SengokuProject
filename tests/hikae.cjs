...
(async () => {
  ...
  /* 城が落ちれば、その城の守兵が家の兵に加わる。だから減りは損害より小さくなる。
     ...
     この物差しを「減りが七千人を下回ること」という頭数で置いていたが、それでは
     軍の大きさに追随しない。武将を増やして家中が厚くなると、寄せる兵も戦の
     規模も大きくなるので、控えが正しく帰っていても七千を超える。実際、
     武将を千百四十五名まで増やしたところ、三万で寄せて七千四十六人が減り、
     盤の損害は七千四十四人であった。一人残らず盤の上で説明がついている。

     測るべきは頭数ではなく割である。控えが消えていたころは、二万のうち
     一万一千（五割五分）が失せた。いまは三万のうち七千（二割三分）である。 */
  確('勝っても控えの兵が消えない', 減 < 前 * 0.35,
    `家の兵 ${前} → ${後}（${減 >= 0 ? '減' : '増'}${Math.abs(減)}人＝${(減 / 前 * 100).toFixed(0)}%／盤の損害 ${討たれた == null ? '―' : 討たれた + '人'}）`);
