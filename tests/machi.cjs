...
/* ------------------------ 二、家紋 */
{
  ...
  /* もとは「家は百十三ある」と数を決め打ちにしていたが、大名家を増やすたびに
     倒れる。この段が守りたいのは数ではなく紋である。数は「盤が読めているか」を
     見るための下限だけにして、代わりに「紋の無い家がないこと」を見る。 */
  確('家が百を超える（盤が読めている）', 家.length >= 100, `${家.length}家`);
  /* アイヌの三家には紋を置かない。イトクパと呼ぶ家印はあるが、和の家紋とは
     別のものであって、木瓜や巴の類を当てるのは誤りである。
     琉球王国の「左御紋」は左三つ巴であるから、これは紋として置いた。 */
  const 紋無 = 家.filter((f) => !f.mon).map((f) => f.name || f.id);
  確('和の家は、すべて紋を持つ（アイヌを除く）',
    紋無.every((n) => /アイヌ/.test(n)),
    紋無.length ? `紋を持たぬのは ${紋無.join('・')}` : 'なし');
  ...
  /* すべての家の紋が、既定の丸ではなく、その家の図柄で描けること（GDD 13.1）。

     紋の名は修飾を伴う。「丸に三つ柏」も「三つ柏」も「柏」も、図柄の核は
     柏である。ところが drawMon は名をそのまま照らしていたので、核が同じでも
     名が違えば既定の丸になった。増補で立てた十五家の紋が描けず、みな同じ丸に
     見えていた。城を攻め取っても紋が変わらぬように見えたのは、これである
     （本山「丸に三つ柏」も、奪った長宗我部「酢漿草」も、片方が丸のまま）。

     いま、名を核へ解いてから照らす。まず名そのもの、次に「丸に」を外した
     もの、次に数の修飾（三つ・七つ・五七・違い）を外したもの、最後に図柄の
     近い紋へ寄せる。アイヌ三家に紋が無いのは正しい（和の家紋の習いがない）。 */
  /* 描いた中身で判ずる。名を照らすだけでは、drawMon がそれを使っている
     かどうかが分からない（実際、核へ解く手を外しても名の照合だけは通った）。
     知らぬ名を描かせて既定の丸の手順を控え、それと同じ手順にならないことを
     見る。 */
  const 手順 = (m) => {
    const 記 = [];
    const 台 = { save: 1, restore: 1, translate: 1, beginPath: 1, closePath: 1 };
    const 筆 = new Proxy({}, { get: (t, p) => {
      const k = String(p);
      if (k === 'measureText') return () => ({ width: 20 });
      if (k.startsWith('create')) return () => ({ addColorStop: () => {} });
      return (...a) => { if (!台[k]) 記.push(k + ':' + a.map((v) => (typeof v === 'number' ? v.toFixed(1) : v)).join(',')); };
    } });
    try { H.drawMon(筆, m, 0, 0, 12, '#000', '#fff'); } catch (e) { return 'x' + e.message; }
    return 記.join('|');
  };
  const 既定 = 手順('この世に無い紋');
  const 丸 = [];
  for (const f of Object.values(H.FACTIONS)) {
    if (!f.mon) continue;                              // アイヌ三家
    if (手順(f.mon) === 既定) 丸.push(`${f.name}「${f.mon}」`);
  }
  確('紋を持つ家は、みな自家の図柄で描ける（既定の丸にならない）', 丸.length === 0,
    丸.slice(0, 6).join('　') || `${Object.values(H.FACTIONS).filter((f) => f.mon).length}家`);
  確('修飾を剥がしても、核の図柄で描かれる',
    手順('丸に三つ柏') !== 既定 && 手順('七つ酢漿草') === 手順('酢漿草')
    && 手順('五七桐') === 手順('桐'),
    '丸に三つ柏→柏 ／ 七つ酢漿草→酢漿草 ／ 五七桐→桐');
   ...
   五隊も出せば城の周りが一杯になり、横に並べて門へ押すのが精一杯であった。 */
{
  ...
  /* 門の数。山城は少ない。
     平城の門を一つ増やしてみたが、寄せ手の隊が門の数だけ散り、かえって
     落ちにくくなった（一万四千の兵で守兵五百の城が落ちなくなった）。
     門の数は元のままとし、広さと堀と縄張りで平城らしさを出している。 */
  確('山城は門が少ない', 山.門 <= 平.門 + 1 && 山.m.gates.length <= 7,
    `平城${平.門}門 ／ 山城${山.門}門`);
