...
}
{
  ...
  /* 終わり方に「引き際」が加わった（GDD 8.8）。

     これまでは、負けた側が盤から消え去るまで戦が続いた。隊ごとに崩れる
     仕掛けはあるが、崩れた隊は盤の上で息をつくので、勝った側が追い回して
     削り続ける。負けた側は初めの兵の一割まで減ってから終わっていた。

     いま、軍を預かる者は支えきれぬと見れば退く。兵が二割を切るか、残る隊の
     士気が尽きたら軍を退く。隊ごとにも、兵が一割を切るか士気が尽きれば
     戦場を離れる。ゆえに終わり方が一つ増える。 */
  const 訳 = {};
  for (const r of 出) {
    const 記 = r.b.log.map((x) => x.text).join(' ');
    const k = /軍を退いた/.test(記) ? '引き際'
      : /総崩れ/.test(記) ? '総崩れ' : /士気が尽きた/.test(記) ? '士気尽き'
      : /兵が尽きた|隊が尽きた/.test(記) ? '兵尽き'
        : r.b.captured ? '本丸を押さえた' : r.終 >= r.日暮 ? '日暮れ' : 'その他';
    訳[k] = (訳[k] || 0) + 1;
  }
  確('戦の終わりは、日暮れ・兵尽き・士気尽き・総崩れ・引き際・落城のいずれか',
