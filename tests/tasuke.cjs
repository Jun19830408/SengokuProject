/* 従属した家を救う（GDD 9.2 / 12.1）。

   従属の家から援軍を求められて後詰を出し、囲みを打ち払って勝った――ところが
   そのまま、救ったはずの城を攻め落としてしまった。

   因は、囲みを解く戦も「城を的とする合戦」の形で立てていたことである
   （startBattle に城を渡し、名だけ「◯◯の囲み」と改めていた）。勝ったあとの
   始末が城を的と見なすので、味方の城をそのまま落とす。援軍が城を奪うなら、
   誰も援軍を頼めない。

   ここでは、従属の城を救って勝ったとき、その城が従属家のもとに残ることを見る。
...
const { createRoot, act, App, React, initState, findPath, 解す: A解す } = require(path.join(__dirname, '..', 'build', 'harness.cjs'));
...
/* 隣り合う他家の城を一つ選び、その家を自家の従属とする。
   救うのは自家の城ではなく、従属した家の城である。 */
let 出陣元 = null, 味方先 = null, 最短 = 99;
for (const a of 自城) {
  for (const b of s.castles) {
    if (b.faction === s.player) continue;
    const p = findPath(a.id, b.id);
    if (p && p.length >= 2 && p.length < 最短) { 出陣元 = a; 味方先 = b; 最短 = p.length; }
  }
}
if (!出陣元 || !味方先) { console.log('★隣り合う他家の城が見つからない'); console.log('エラー: 仕込めず'); process.exit(1); }
const 従属家 = 味方先.faction;
s.relations[[s.player, 従属家].sort().join('|')] =
  { state: '従属', master: s.player, trust: 90, until: null };
...
/* 囲む敵は、将のいる城から選ぶ。将のいない軍は読み込みのときに繕われて消え、
   囲みも一緒に落ちる（将の無い軍を繕う）。一度それで仕込みが空になった。 */
let 敵 = null;
for (const d of s.castles) {
  if (d.faction === s.player || d.faction === 従属家) continue;
  if (!s.generals.some((x) => x.at === d.id && x.faction === d.faction && !x.captive)) continue;
  const p = findPath(d.id, 味方先.id);
  if (p && p.length <= 3) { 敵 = d; break; }
}
if (!敵) { console.log('★囲める敵城が見つからない'); console.log('エラー: 仕込めず'); process.exit(1); }
...
console.log(`仕込み: ${出陣元.name}（自家）から出陣。従属の ${s.factions[従属家].name}・${味方先.name} は`
  + `${敵 ? `${s.factions[敵.faction].name}に囲まれている` : '無事'}`);
...
(async () => {
  ...
  確('後詰として進発できる', 出た);
  {
    /* 戦国記の末尾を読む。「後詰が発した」なら救いの筋、「攻める」なら戦役の筋。 */
    await rc('戦国記'); await flush();
    const 記 = (document.querySelector('.card') || {}).textContent || '';
    await rc('閉じる'); await flush();
    const 末 = 記.replace(/\s+/g, ' ').match(/[^。]*(後詰が発した|を攻める|援軍を送る)[^。]*。/);
    console.log('    （出陣の記）', 末 ? 末[0] : '（見当たらない）');
  }
  ...
  if (btn('委ねて結果を見る')) {
    ...
    // 戦の顛末は戦国記に残る。画面の文字は移り変わるので、記録のほうを見る。
    await rc('戦国記'); await flush();
    const 後 = document.body.textContent;
    await rc('閉じる'); await flush();
    /* 城方が討って出るかどうかは運による。出れば「討って出た」と残り、
       出なければ囲みを解く野戦の記だけが残る。どちらでも筋は通っている。
       盤を広げたら、出てこない目のほうが増えた。決着がついていればよしとする。 */
    /* ここが肝である。勝っても、救った城は従属家のもとに残らねばならない。 */
    const 盤 = (() => { try { return JSON.parse(A解す(蔵.get('sengoku:save1'))).state; } catch (e) { return null; } })();
    const 後城 = 盤 && 盤.castles.find((c) => c.id === 味方先.id);
    確('救った城は従属家のものであり続ける',
      !後城 || 後城.faction === 従属家,
      後城 ? `${後城.name} は ${(盤.factions[後城.faction] || {}).name} のもの` : '（盤が読めない）');
    /* 囲みが解けるのは、寄せ手を退けたときである。討って出た城方が敗れれば
       囲みは残る――それは筋どおりで、ここで見たいのは「救った城が味方のもとに
       残ること」のほうである。解けたか否かは併せて控えるに留める。 */
    const 囲 = 盤 && (盤.sieges || []).some((x) => x.castleId === 味方先.id);
    console.log(`    （囲みは${囲 ? 'まだ残っている' : '解けた'}）`);
    確('救った城が寄せ手のものになっていない',
      !後城 || 後城.faction !== (敵 ? 敵.faction : null),
      後城 ? `${(盤.factions[後城.faction] || {}).name}` : '');
    確('後詰の戦の顛末が記される',
      /討って出た/.test(後) || /囲み/.test(後) || /後詰/.test(後) || /退けられ/.test(後)
      || /打ち払/.test(後) || /合戦/.test(後),
      (後.match(/[^\s。]{0,24}(討って出た|囲み|後詰)[^。]{0,24}。/) || [''])[0] || 後.replace(/\s+/g, ' ').slice(0, 80));
  }
