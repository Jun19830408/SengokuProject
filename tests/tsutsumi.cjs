...
(async () => {
     ...
     もう一度だけ書いてみる。中身は一字も変えない。 */
  {
    ...
    /* 詰め直しで枠が消えないこと。ここが此の段の要である。

       盤が育って一枠が一.二六MBから一.五六MBになったとき、詰め直しは
       「圧したもので上書きする」だけであったので、置き換えたあとの総量も
       なお溢れていて一枠も通らなかった（六.三MBの棚で六度撥ねられ、
       一バイトも減らなかった）。

       そこで「下ろしてから置き直す」形に改めたところ、こんどは撥ねられて
       元の記録を書き戻すときに、その書き戻しもまた撥ねられた。棚はなお
       溢れたままだからである。下ろした枠が、そのまま失われた。

       空きが出来たかどうかだけを見ていては、この失い方は捕まらない。
       入れておいた四枠が、四枠とも読めることを確かめる。 */
    const 生きた = [];
    for (let i = 1; i <= 4; i++) {
      const d = await A.loadGame(A.枠の鍵(i));
      if (d && d.state && (d.state.castles || []).length === s.castles.length) 生きた.push(i);
    }
    確('詰め直しで枠が一つも消えない', 生きた.length === 4,
      `四枠のうち ${生きた.length}枠が読める`);
  }
