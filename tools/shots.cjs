...
const 場面 = [
  ...
  { key: 'field-melee', 盤: '野戦', 手: ['押:続きから', '待:1.4', '押:正面から当たる', '待:0.8',
    '押:合戦開始', '待:0.6', '押:全軍接戦', '待:0.5', '押:通常', '噛待:220,60', '待:22',
    '押:停止', '待:0.5', '押:停止', '待:0.5', '寄:0', '待:1.2'],
...
(async () => {
  ...
  /* 噛み合いがいちばん激しくなるまで待つ。盤に組の数を尋ね、増えるあいだは
     待ち、頭打ちになったところで止める。ただ待つだけでは、近づいている途中か、
     もう追撃に移ったあとかが分からない。 */
  const 噛待 = async (限り, 要る) => {
    let 最多 = 0, 伸びず = 0;
    for (let i = 0; i < 限り * 2; i++) {
      await 眠(0.5);
      const n = typeof window.__噛み === 'function' ? window.__噛み() : 0;
      if (n > 最多) { 最多 = n; 伸びず = 0; } else { 伸びず++; }
      /* 要る数だけ噛み合い、しばらく伸びなくなったら、そこが盛りである。
         早く切り上げると近づいている途中が写り、遅らせると追撃になる。 */
      if (最多 >= 要る && 伸びず >= 8) break;
    }
    return 最多;
  };
  ...
  ${手.map((h) => {
    ...
    if (k === '噛待') { const [a2, b2] = v.split(','); return `await 噛待(${Number(a2)}, ${Number(b2 || 20)});`; }
    return '';
  }).join('\n  ')}
